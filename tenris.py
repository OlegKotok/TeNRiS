...
class TenrisGame:
    ...
    def load_assets(self):
        if self.headless:
            ...
            self.sounds = {
                k: None for k in ["rot", "move", "clear", "down", "rotate_key", "hit"]
            }
        ...
        try:
            self.bg_image = pygame.image.load(
                resource_path(os.path.join("texture", "picture.jpg"))
            ).convert()
        ...
        try:
            self.logo = pygame.image.load(
                resource_path(os.path.join("texture", "logo.bmp"))
            ).convert()
        ...
        try:
            if pygame.mixer.get_init():
                pygame.mixer.music.load(
                    resource_path(os.path.join("sounds", "crasy frog.mid"))
                )
    ...
    def check_fishka(self, sx, sy, tetr=None):
        if tetr is None:
            tetr = self.current_fig
        for x in range(1, 4):
            for y in range(1, 4):
                if tetr[x][y] > 0:
                    mx, my = sx + x, sy + y
                    if mx < 1 or mx > 16 or my > 24:
                        return True
                    if my >= 1 and self.map[mx][my] > 0:
                        return True
        return False
    ...
    def move_down(self, megadown=False):
        if megadown:
            while not (
                self.check_fishka(self.fig_sx, self.fig_sy + 1) or self.check_lim()
            ):
                self.fig_sy += 1
            self.copy_to_map()
            self.spawn_figure()
    ...
    def test_logic(self):
        ...
        for x in range(1, 16):
            for y in range(1, 26):
                if self.map[x][y] != 0:
                    # Horizontal
                    if self.map[x + 1][y] != 0 and (
                        self.map[x][y] + self.map[x + 1][y] == 10
                    ):
                        self.metki[x][y] = True
                    ...
                    # Vertical
                    if self.map[x][y + 1] != 0 and (
                        self.map[x][y] + self.map[x][y + 1] == 10
                    ):
                        self.metki[x][y] = True
    ...
    def draw(self):
        ...
        if self.font_main:
            ...
            if self.game_over:
                msg = self.font_score.render(
                    "GAME OVER - Press R", True, (255, 255, 255)
                )
                if msg:
